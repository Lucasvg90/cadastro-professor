...
public class ProfessorRepository {
    ...
    public List<Professor> findAll() {
        return jdbcTemplate.query(
                "SELECT id, nome, matricula, disciplina, observacao, ativo FROM PROFESSORES ORDER BY id",
                PROFESSOR_ROW_MAPPER);
    }

    public Optional<Professor> findById(int id) {
        return jdbcTemplate.query(
                "SELECT id, nome, matricula, disciplina, observacao, ativo FROM PROFESSORES WHERE id = ?",
                PROFESSOR_ROW_MAPPER, id).stream().findFirst();
    }

    public Professor save(Professor professor) {
        String sql = "INSERT INTO PROFESSORES (nome, matricula, disciplina, observacao, ativo) VALUES (?, ?, ?, ?, ?)";
        ...
        jdbcTemplate.update(connection -> {
            ...
            ps.setString(4, professor.getObservacao());
    ...
    public Professor update(Professor professor) {
        jdbcTemplate.update(
                "UPDATE PROFESSORES SET nome = ?, matricula = ?, disciplina = ?, observacao = ?, ativo = ? WHERE id = ?",
                professor.getNome(),
